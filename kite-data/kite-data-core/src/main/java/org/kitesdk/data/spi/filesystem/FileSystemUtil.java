...
public class FileSystemUtil {
   ...
   */
    ...
    private static boolean cleanlyDelete(FileSystem fs, Path root, Path path, boolean useTrash) {
        ...
        try {
            ...
            if (relativePath.isAbsolute()) {
                ...
                deleted = useTrash ? Trash.moveToAppropriateTrash(fs, path, fs.getConf()) : fs.delete(path, true);
            } else {
                // the is relative to the root path
                Path absolute = new Path(root, relativePath);
                LOG.debug("Deleting path {}", absolute);
                deleted = useTrash ? Trash.moveToAppropriateTrash(fs, absolute, fs.getConf()) : fs.delete(absolute, true);
                // iterate up to the root, removing empty directories
                for (Path current = absolute.getParent(); !current.equals(root) && !(current.getParent() == null); current = current.getParent()) {
    ...
    public static PartitionStrategy strategy(FileSystem fs, Path location) throws IOException {
        ...
        List<Pair<String, Class<? extends Comparable>>> pairs = visit(new GetPartitionInfo(), fs, location);
    ...
    private abstract static class PathVisitor<T> {
    ...
    static {
        ...
        try {
            isSymlink = new DynMethods.Builder("isSymlink").impl(FileStatus.class).buildChecked();
    ...
    private static class GetPartitionInfo extends PathVisitor<List<Pair<String, Class<? extends Comparable>>>> {
        ...
        List<Pair<String, Class<? extends Comparable>>> directory(FileSystem fs, Path path, List<List<Pair<String, Class<? extends Comparable>>>> children) throws IOException {
            ...
            for (List<Pair<String, Class<? extends Comparable>>> child : children) {
                ...
                for (int i = 0; i < child.size(); i += 1) {
                    if (accumulated.size() > i) {
                        Pair<String, Class<? extends Comparable>> pair = merge(accumulated.get(i), child.get(i));
            ...
            accumulated.add(0, new Pair<String, Class<? extends Comparable>>(name, dataClass(value)));
            return accumulated;
        }
        ...
        Result directory(FileSystem fs, Path path, List<Result> children) throws IOException {
            ...
            for (Result child : children) {
                if (child instanceof Result.Unknown) {
                    ...
                    // always add table in case not compatible later
                    tables.add(table);
   ...
   */
    public static boolean supportsRename(URI fsUri, Configuration conf) {
        ...
        return conf.getBoolean(FileSystemProperties.SUPPORTS_RENAME_PROP, !(fsUriScheme.equalsIgnoreCase("s3n") || fsUriScheme.equalsIgnoreCase("s3a")));
    }
}
