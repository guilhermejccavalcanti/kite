 ...
 */
@Immutable
class FileSystemView<E> extends AbstractRefinableView<E> implements InputFormatAccessor<E>, LastModifiedAccessor, SizeAccessor, Signalable<E> {
    ...
    public DatasetReader<E> newReader() {
        checkSchemaForRead();
        AbstractDatasetReader<E> reader = new MultiFileDatasetReader<E>(fs, pathIterator(), dataset.getDescriptor(), constraints, getAccessor());
        reader.initialize();
        return reader;
    }
    ...
    public DatasetWriter<E> newWriter() {
        ...
        if (dataset.getDescriptor().isPartitioned()) {
            ...
            writer = FileSystemWriter.newWriter(fs, root, -1, -1, /* get from descriptor */
            dataset.getDescriptor(), this.getAccessor().getWriteSchema());
        }
        writer.initialize();
        return writer;
    }
        ...
        if (dataset.getDescriptor().isPartitioned()) {
            return new Iterable<PartitionView<E>>() {
                ...
                public Iterator<PartitionView<E>> iterator() {
                    return Iterators.transform(partitionIterator(), new Function<StorageKey, PartitionView<E>>() {
    ...
    public boolean moveToTrash() {
        if (!constraints.alignedWithBoundaries()) {
            throw new UnsupportedOperationException("Cannot cleanly move view to trash: " + this);
        }
   ...
   */
    ...
    private FileSystemPartitionIterator partitionIterator() {
        ...
        try {
            return new FileSystemPartitionIterator(fs, root, descriptor.getPartitionStrategy(), descriptor.getSchema(), getKeyPredicate());
    ...
    boolean deleteAllUnsafe(boolean useTrash) {
        ...
        if (dataset.getDescriptor().isPartitioned()) {
            for (StorageKey key : partitionIterator()) {
                deleted = (useTrash ? FileSystemUtil.cleanlyMoveToTrash(fs, root, key.getPath()) : FileSystemUtil.cleanlyDelete(fs, root, key.getPath())) || deleted;
                if (listener != null) {
                    // the relative path is the partition name, so we can simply delete it
                    // in Hive
                    listener.partitionDeleted(dataset.getNamespace(), dataset.getName(), key.getPath().toString());
                }
            }
        } else {
            for (Path path : pathIterator()) {
                deleted = (useTrash ? FileSystemUtil.cleanlyMoveToTrash(fs, root, path) : FileSystemUtil.cleanlyDelete(fs, root, path)) || deleted;
            }
        }
        return deleted;
    }
